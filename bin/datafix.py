...
from app.tx import remove_db_tx, get_db_tx, get_tx_db_block
...
def check_cnt_txes(conn):
    for block in itercol(conn, conn.block, 'check_block._id', 1000):
        ...
        if cnt_txes != block['cntTxes']:
            logging.warn('cnt txes mismatch %s vs. %s', block['hash'].encode('hex'), cnt_txes, block['cntTxes'])

def add_spt(conn):
    for dtx in itercol(conn, conn.tx, 'spt.tx._id', 10000):
        for i, input in enumerate(dtx['vin']):
            ...
            if not source_tx:
                logging.warn('source tx not found %s', input['hash'], )
                continue

            output = source_tx['vout'][input['n']]
            if not output:
                logging.warn('cannot find output for %s at %s', input['hash'], input['n'])
...
def cleanup_txes(conn):
    for dtx in itercol(conn, conn.tx, 'cleanup_txes._id', 10000):
        if generated_seconds(dtx['_id'].generation_time) > 86400:
            b, _ = get_tx_db_block(conn, dtx)
            if not b:
                logging.warn('tx %s is outdated, cleaning up', dtx['hash'].encode('hex'))
                with transaction(conn) as conn:
